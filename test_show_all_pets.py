...
def test_show_all_pets(driver):
    driver.find_element(By.ID,'email').send_keys('lukira.greig@gmail.com')
    driver.find_element(By.ID,'password').send_keys('554482Q')
    driver.find_element(By.CSS_SELECTOR, 'button[type=submit]').click()
    assert driver.find_element(By.TAG_NAME,'h1').text == "PetFriends"


def test_all_my_pets(driver):
    driver.find_element(By.ID,'email').send_keys('lukiar.greig@gmail.com')
    driver.find_element(By.ID,'password').send_keys('554482Q')
    driver.find_element(By.CSS_SELECTOR,'button[type=submit').click()
    assert driver.find_element(By.TAG_NAME,'h1').text == "PetFriends"
    time.sleep(1)
